...
{
    mModule = torch::jit::load("/home/slam/YOLOv5_ORBSLAM3_dense_stereo/my_yolov5.pt");

    std::ifstream f("/home/slam/YOLOv5_ORBSLAM3_dense_stereo/my_classes.names");
    std::string name = "";
...
bool YoloDetection::Detect()
{
    ...
    std::vector<torch::Tensor> dets = YoloDetection::non_max_suppression(preds, 0.25, 0.45);
    if (dets.size() > 0)
    {
        // Visualize result
